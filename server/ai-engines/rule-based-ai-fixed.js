// Professional Rule-Based AI Engine - Fixed Version
...
export async function ruleBasedAnalysis(content) {
  ...
  // 2. Detect spam words - Keep actual words for proper highlighting
  const spamWords = [
  ...
  spamWords.forEach(word => {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      if (!taggedContent.includes(`<spam_words>${actualWord}</spam_words>`)) {
        taggedContent = taggedContent.replace(actualWord, `<spam_words>${actualWord}</spam_words>`);
        spamWordsCount++;
  ...
  // 3. Detect fluff words - Keep actual words for proper highlighting
  const fluffWords = [
  ...
  fluffWords.forEach(word => {
    ...
    while ((match = regex.exec(content)) !== null) {
      const actualWord = match[0];
      if (!taggedContent.includes(`<fluff>${actualWord}</fluff>`)) {
        taggedContent = taggedContent.replace(actualWord, `<fluff>${actualWord}</fluff>`);
        fluffWordsCount++;
